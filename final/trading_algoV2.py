...
'''
data = pd.read_csv(YOUR_PATH+'/full_data.csv')
data['Date'] = data['Unnamed: 0']
data = data.drop(['Unnamed: 0'], axis=1)
data = data.set_index(['Date'])
data.index = pd.to_datetime(data.index)           # Setting date as entry index
data = data.dropna().copy()
test_data = data.loc['2020-01-01':'2020-12-31']   # Store data for dates we will be running the algo on
...
algo_df.to_csv(YOUR_PATH+'/algo_dfV2-2020.csv', encoding='utf-8')
